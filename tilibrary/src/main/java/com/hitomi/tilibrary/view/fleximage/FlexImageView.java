...
public class FlexImageView extends PhotoView {
    ...
    private int mState = STATE_TRANS_NORMAL;
...
//		mOriginalLocationY = mOriginalLocationY - getStatusBarHeight(getContext());
    ...
    private void initTransform() {
        ...
        float xSScale = mOriginalWidth / ((float) mDrawable.getIntrinsicWidth());
        float ySScale = mOriginalHeight / ((float) mDrawable.getIntrinsicHeight());
        ...
        float xEScale = getWidth() / ((float) mDrawable.getIntrinsicWidth());
        float yEScale = getHeight() / ((float) mDrawable.getIntrinsicHeight());
        ...
        float bitmapEndWidth = mDrawable.getIntrinsicWidth() * mTransfrom.endScale;// 图片最终的宽度
        float bitmapEndHeight = mDrawable.getIntrinsicHeight() * mTransfrom.endScale;// 图片最终的宽度
    ...
    private void getCenterCropMatrix() {
		...
		/* 下面实现了CENTER_CROP的功能 */
        float xScale = mOriginalWidth / ((float) mDrawable.getIntrinsicWidth());
        float yScale = mOriginalHeight / ((float) mDrawable.getIntrinsicHeight());
        float scale = xScale > yScale ? xScale : yScale;
        mSmoothMatrix.reset();
        mSmoothMatrix.setScale(scale, scale);
        mSmoothMatrix.postTranslate(-(scale * mDrawable.getIntrinsicWidth() / 2 - mOriginalWidth / 2), -(scale * mDrawable.getIntrinsicHeight() / 2 - mOriginalHeight / 2));
    }
    ...
    private void getBmpMatrix() {
        ...
        mSmoothMatrix.postTranslate(-(mTransfrom.scale * mDrawable.getIntrinsicWidth() / 2 - mTransfrom.rect.width / 2),
                -(mTransfrom.scale * mDrawable.getIntrinsicHeight() / 2 - mTransfrom.rect.height / 2));
    }
